...
#' <snip>
#' 
#' @param file path to knownGene file
#' @return see import.ucsc.file
#' 
#' @author Mark Cowley, 8 August 2006
#' @export
...
#' <snip>
#' 
#' @param file path to kgXref file
#' @return see import.ucsc.file
#' 
#' @author Mark Cowley, 8 August 2006
#' @export
#' @importFrom mjcbase file.gunzip
import.kgXref <- function(file="/biomirror/hgdownload.cse.ucsc.edu/tables/kgXref.gz") {
	tmp <- paste0(tempfile(pattern="ucsc"), ".gz")
	file.copy(file, tmp)
	file.gunzip(tmp)
	tmp <- sub("\\.gz", "", tmp)
...
#' import UCSC-style file
#' 
#' File from UCSC tend to have the column names in the first line preceeded
...
#' Download knownGeneToLocusLink from UCSC Table Browser to: Import the
#' knownToLocusLink file, downloaded from the UCSC Table Browser. 
#' 
#' @param taxid the numerical taxonID
#' @param files a list with your file name in it.
#' 
